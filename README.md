...
- **PID 取图** — `#pid 作品ID...`，按指定 PID 精确获取作品
- **合并转发** — 搜索结果以合并转发消息发送，包含图片、标题、作者和 ID
- **代理支持** — 支持 HTTP/HTTPS/SOCKS5 代理，解决无法直连 Pixiv 的问题
- **网络重试** — API 请求和图片下载均支持自动重试，提升网络不稳定环境下的可靠性
...
- **懒加载登录** — 首次使用命令时自动登录 Pixiv，无需重启插件
...
| `#p站` | 随机推荐 Pixiv 插画 |
| `#p站 关键词` | 搜索关键词并返回插画 |
| `#pid 12345678 87654321` | 按 PID 获取一个或多个指定作品 |
...
| PID 命令最大数量 | `5` | `pid` 命令一次最多可携带的 PID 数量（1-10） |
| 代理地址 | 空（直连） | HTTP/HTTPS/SOCKS5 代理地址，如 `http://127.0.0.1:7890` |
...
`pid` 命令不会检查输入的数字 PID 本身，但会对对应作品继续执行 R-18、敏感内容和违禁词过滤。
